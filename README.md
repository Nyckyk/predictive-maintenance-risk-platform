...
The platform predicts **Remaining Useful Life (RUL)** from multivariate sensor data, converts predictions into operational maintenance risk categories, estimates short horizon HIGH risk probability, and explores illustrative maintenance vs failure cost scenarios through an interactive Streamlit dashboard.
...
- **Fleet Risk**: operational fleet prioritisation using predicted RUL and HIG risk probability
...
The capped-RUL and uncapped-RUL formulations are different targets, so their MAE values should not be interpreted as a direct percentage improvement.
...
The operational threshold is intentionally more conservative than the reference 30-cycle definition in order to detect additional near failure engines.

---

## Risk-Region Error Analysis

Prediction error was also evaluated separately by actual maintenance risk region.
...
Calibration was also inspected by probability bucket.
...
The raw NASA dataset can be kept out of version control and added locally.
...
- the official test set has been inspected during project development, including earlier baseline evaluation, so it should not be described as a completely untouched final holdout
...
- refactor the pipeline into reusable modules
- add automated unit and integration tests
- add GitHub Actions CI
- Dockerise the application
- persist trained models
- add model/version metadata
- add additional C-MAPSS subsets such as FD002–FD004
- investigate time series feature engineering
- add uncertainty intervals around RUL predictions
- evaluate calibration methods on larger validation datasets
...
That combination demonstrates predictive modelling, engineering reasoning, model validation, probability analysis, financial scenario modelling and application development within one project.
